...
1. From the Cloud Shell pane, deploy the two Azure VMs hosting Windows Server 2016 Datacenter into the first virtual network by running:
   ...
   ```
   New-AzResourceGroupDeployment -ResourceGroupName az3000501-LabRG -TemplateFile $HOME/azuredeploy05.json -TemplateParameterFile $HOME/azuredeploy05.parameters.json
   ```
